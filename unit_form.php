...
if (($mode === 'edit' || $mode === 'copy' || $mode === 'delete') && $id > 0) {
    $stmt = $conn->prepare("SELECT unit, note FROM unit WHERE unit_id = ?");
    ...
    if (!$r) {
        ...
        $origUnit = (string)$r['unit'];
        $values['unit'] = ($mode === 'copy') ? '' : (string)$r['unit'];
        $values['note'] = ($mode === 'copy') ? '' : (string)$r['note'];
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $values['unit'] = trim((string)($_POST['unit'] ?? ''));
    $values['note'] = trim((string)($_POST['note'] ?? ''));

    if ($mode !== 'delete' && $values['unit'] === '') {
        $errors[] = 'Поле «Ед. измерения» обязательно для заполнения.';
        $focusField = 'unit';
    }
    ...
    if (empty($errors)) {
        if ($mode === 'delete') {
            ...
            $stmt->execute();
            $stmt->close();
            $conn->query("DELETE FROM marks WHERE tbl = 'unit' AND row_id = " . (int)$id);
            if ($isAjax) {
                header('Content-Type: application/json; charset=utf-8');
                echo json_encode(['ok' => true, 'mode' => $mode, 'id' => $id, 'name' => $origUnit]);
                exit;
            }
        ...
        if ($mode === 'new' || $mode === 'copy') {
            $baseUnitName = $values['unit'];
            $insertCounter = 1;
            $stmt = $conn->prepare("INSERT INTO unit (unit, note) VALUES (?, ?)");
            bind_auto($stmt, [$values['unit'], $values['note']]);
            $stmt->execute();
            $dup = (mysqli_errno($conn) === 1062);
            while ($dup && $insertCounter < 100) {
                $values['unit'] = $baseUnitName . ' (' . $insertCounter . ')';
                bind_auto($stmt, [$values['unit'], $values['note']]);
                $stmt->execute();
                $dup = (mysqli_errno($conn) === 1062);
                $insertCounter++;
            ...
            $stmt->close();
            if ($isAjax) {
                $pageOfNew = computePageOfNew($conn, 'unit', 'id', 'id', 'asc', $newId, $newId, '');
                header('Content-Type: application/json; charset=utf-8');
                echo json_encode(['ok' => true, 'mode' => $mode, 'id' => $newId, 'name' => $values['unit'], 'page' => $pageOfNew]);
                exit;
            }
            header('Location: unit.php');
            exit;
        ...
        if ($mode === 'edit') {
            $baseUnitName = $values['unit'];
            $editCounter = 1;
            $stmt = $conn->prepare("UPDATE unit SET unit = ?, note = ? WHERE unit_id = ?");
            bind_auto($stmt, [$values['unit'], $values['note'], $id]);
            $stmt->execute();
            $dup = (mysqli_errno($conn) === 1062);
            while ($dup && $editCounter < 100) {
                $values['unit'] = $baseUnitName . ' (' . $editCounter . ')';
                bind_auto($stmt, [$values['unit'], $values['note'], $id]);
                $stmt->execute();
                $dup = (mysqli_errno($conn) === 1062);
                $editCounter++;
            ...
            if ($isAjax) {
                header('Content-Type: application/json; charset=utf-8');
                echo json_encode(['ok' => true, 'mode' => $mode, 'id' => $id, 'name' => $values['unit']]);
                exit;
            }
            header('Location: unit.php');
            exit;
...
$titles = [
    'new'    => 'Ед. измерения (новая)',
    'edit'   => 'Ед. измерения: ' . $origUnit,
    'copy'   => 'Ед. измерения: ' . $origUnit . ' (копия)',
    'delete' => 'Ед. измерения: ' . $origUnit . ' (удаление)',
];
...
ob_start();
?>
<h2 class="page-title<?= $mode === 'delete' ? ' page-title--delete' : '' ?>"><img src="img/unit.png" alt="" /> <?= h($pageTitle) ?></h2>
<form class="form<?= $mode === 'delete' ? ' form--delete' : '' ?>" method="post" action="unit_form.php" autocomplete="off" data-form-modal>
...
<?= render_form_actions(
    $mode === 'delete'
        ...
        : [render_btn_primary('img/save.png', 'Сохранить', ['type'=>'submit','formnovalidate'=>true,'tabindex'=>'-1']),
           render_btn_link_icon_text('img/cancel.png', 'Отменить', 'unit.php', ['class'=>'btn-secondary','tabindex'=>'-1'])]
) ?>
<?php endif; ?>

<?= render_field('Единица измерения', render_input('text', 'unit', $values['unit'], [
        'id' => 'unit-name-input',
        'required' => !$isReadonly,
        'readonly' => $isReadonly,
...
<?= render_form_actions(
    $mode === 'delete'
        ? [render_btn_danger('img/delete.png', 'Удалить', ['type'=>'submit','formnovalidate'=>true]),
           render_btn_link_icon_text('img/cancel.png', 'Отменить', 'unit.php', ['class'=>'btn-secondary'])]
