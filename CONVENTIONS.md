...
## 9. Форматирование чисел (decimal)

Во всей системе действует правило:

- **Убирать нули после десятичной точки.** Например: `1.000` → `1`, `1.500` → `1.5`, `15.0` → `15`.
- **Если значение равно нулю — не выводить.** Например: скидка 0%, сумма 0,00 — пустая строка.

Применять ко всем числовым полям с плавающей точкой: количество, цена, скидка, сумма, сумма_скидки, сумма_ндс.

### Реализация

**PHP** — при формировании данных для таблицы (в `_list` и при возврате `item` после сохранения):
```php
$val = (float)$value;
// если ноль — пустая строка
if ($val == 0) { $item['field'] = ''; }
// иначе убрать хвостовые нули
else { $item['field'] = rtrim(rtrim($value, '0'), '.'); }
```

**JS** — в конфигурации колонки можно добавить `hideZero: true` (работает на уровне `EmbeddedSubTable`):
```js
col.hideZero && (val === 0 || val === '0') ? '' : esc(val)
```

### Исключения
- Цены и курсы валют могут требовать фиксированного числа знаков — в таких случаях правило не применяется.
