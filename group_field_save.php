...
$chk = $conn->prepare("SELECT group_id FROM `group` WHERE group_id = ?");
$chk->bind_param('i', $id);
$chk->execute();
$chk->store_result();
...
$ALLOWED = [
    'name' => ['col' => 'name', 'type' => 's', 'max' => 60],
    'note' => ['col' => 'note', 'type' => 's', 'max' => 120],
];
...
if ($field === 'name' && $value === '') {
    echo json_encode(['ok' => false, 'error' => 'Поле «Название» не может быть пустым']);
    exit;
}
if (mb_strlen($value) > $cfg['max']) $value = mb_substr($value, 0, $cfg['max']);

$stmt = $conn->prepare("UPDATE `group` SET {$cfg['col']} = ? WHERE group_id = ?");
