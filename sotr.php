        ...
        <?php render_table_thead($visibleColumns, $COL_META, $sortLevels, $allRowsMarked, $rowsTotalCount === 0, [
            'thAttrsCallback' => function($cn, $cm) use ($roleFilterIds) {
                if ($cm && !empty($cm['filter']) && $cn === 'role') {
                    return ' data-col="' . h($cn) . '" data-param="role" data-values="' . h(implode(',', $roleFilterIds)) . '"';
                }
                return '';
            },
  ...
  <script>
    ...
    (function () {
      ...
      checkAll.addEventListener('change', function () {
        location.href = 'sotr.php?action=toggleSelectAll' + (search ? '&q=' + encodeURIComponent(search) : '');
      });
      ...
      rowChecks.forEach(cb => cb.addEventListener('change', function () {
        ...
        const to  = cb.checked;
        cb.disabled = true;
        fetch('sotr.php?action=toggleSelect', {
          method: 'POST',
