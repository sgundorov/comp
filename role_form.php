...
if (($mode === 'edit' || $mode === 'copy' || $mode === 'delete') && $id > 0) {
    $stmt = $conn->prepare("SELECT r.role, r.note FROM `role` r WHERE r.role_id = ?");
    ...
    if (!$r) {
        ...
        $origRole = (string)$r['role'];
        $values['role'] = ($mode === 'copy') ? '' : (string)$r['role'];
        $values['note'] = (string)$r['note'];
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $values['role'] = trim((string)($_POST['role'] ?? ''));
    $values['note'] = trim((string)($_POST['note'] ?? ''));

    if ($mode !== 'delete') {
        if ($values['role'] === '') { $errors[] = 'Поле «Роль» обязательно для заполнения.'; $focusField = 'role'; }
    }
    ...
    if (empty($errors)) {
        if ($mode === 'delete') {
            ...
            $stmt->execute();
            $stmt->close();
            $conn->query("DELETE FROM marks WHERE tbl = 'role' AND row_id = " . (int)$id);
            if ($isAjax) {
        ...
        if ($mode === 'new' || $mode === 'copy') {
            $stmt = $conn->prepare("INSERT INTO `role` (role, note) VALUES (?, ?)");
            bind_auto($stmt, [$values['role'], $values['note']]);
            $stmt->execute();
            $newId = (int)$conn->insert_id;
            $stmt->close();
            if ($isAjax) {
                $pageOfNew = computePageOfNew($conn, 'role', 'id', 'id', 'asc', $newId, $newId, '');
                header('Content-Type: application/json; charset=utf-8');
                echo json_encode(['ok' => true, 'mode' => $mode, 'id' => $newId, 'name' => $values['role'], 'page' => $pageOfNew]);
                exit;
            }
            header('Location: role.php');
            exit;
        ...
        if ($mode === 'edit') {
            $stmt = $conn->prepare("UPDATE `role` SET role = ?, note = ? WHERE role_id = ?");
            bind_auto($stmt, [$values['role'], $values['note'], $id]);
            $stmt->execute();
            $stmt->close();
            if ($isAjax) {
                header('Content-Type: application/json; charset=utf-8');
...
$titles = [
    'new'    => 'Роль (новая)',
    'edit'   => 'Роль: ' . $origRole,
    'copy'   => 'Роль: ' . $origRole . ' (копия)',
    'delete' => 'Роль: ' . $origRole . ' (удаление)',
];
...
ob_start();
?>
<h2 class="page-title<?= $mode === 'delete' ? ' page-title--delete' : '' ?>"><img src="img/role.png" alt="" /> <?= h($pageTitle) ?></h2>
<form class="form<?= $mode === 'delete' ? ' form--delete' : '' ?>" method="post" action="role_form.php" autocomplete="off" data-form-modal>
...
<?= render_form_actions(
    $mode === 'delete'
        ...
        : [render_btn_primary('img/save.png', 'Сохранить', ['type'=>'submit','formnovalidate'=>true,'tabindex'=>'-1']),
           render_btn_link_icon_text('img/cancel.png', 'Отменить', 'role.php', ['class'=>'btn-secondary','tabindex'=>'-1'])]
) ?>
<?php endif; ?>

<?= render_field('Роль', render_input('text', 'role', $values['role'], [
        'id' => 'role-name',
        'required' => !$isReadonly,
        'readonly' => $isReadonly,
...
<?= render_form_actions(
    $mode === 'delete'
        ? [render_btn_danger('img/delete.png', 'Удалить', ['type'=>'submit','formnovalidate'=>true]),
           render_btn_link_icon_text('img/cancel.png', 'Отменить', 'role.php', ['class'=>'btn-secondary'])]
