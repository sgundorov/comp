...
if ($rawCols !== '') {
    $searchCols = array_values(array_filter(array_map('trim', explode(',', $rawCols)), function ($k) {
        return in_array($k, ['id', 'unit', 'note'], true);
    }));
}
...
if ($sortRaw !== '') {
    foreach (explode(',', $sortRaw) as $lv) {
        ...
        if (in_array($ck, ['id','unit','note'], true) && in_array($dk, ['asc','desc'], true)) {
...
$COLUMN_DEFAULTS = unit_columns_defaults();
$COL_META = [];
...
$COL_META['id']['value']   = function ($r) { return (int)$r['unit_id']; };
$COL_META['unit']['value'] = function ($r) { return (string)$r['unit']; };
$COL_META['note']['value'] = function ($r) { return (string)$r['note']; };
$PRINT_WIDTHS = unit_columns_widths_print();

$columnsConfig  = load_columns_config($conn, 'unit', $COLUMN_DEFAULTS);
...
$pageCount = 0;
if ($onlyPage) {
    $countSql = "SELECT COUNT(*) AS c FROM unit u $where";
...
$sql = "SELECT u.unit_id, u.unit, u.note FROM unit u $where ORDER BY $orderBy";
if ($onlyPage) {
...
<head>
  <meta charset="UTF-8" />
  <title>Единицы измерения — Печать</title>
  <style>
    body {
...
<body>
  ...
  <div class="print-page">
    <div class="print-page-header">
      <div class="print-page-title">Единицы измерения<?= $onlyPage ? ' &mdash; страница ' . (int)$pageNum : '' ?></div>
