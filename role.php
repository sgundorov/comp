...
$rowsMarkedCount = 0;
foreach ($rows as $r) { if (isset($marks[(int)$r['role_id']])) $rowsMarkedCount++; }
$rowsTotalCount  = count($rows);
$allRowsMarked   = $rowsTotalCount > 0 && $rowsMarkedCount === $rowsTotalCount;
?>
<?php
render_head_start('Роли сотрудников'); ?>
  <style>
    ...
    .data-table tbody tr:hover td.col-role { background: #2c3a4d; }
    .data-table tbody tr.selected td.col-role { background: #3a5a8a; }
    .data-table tbody tr.selected:hover td.col-role { background: #3a5a8a; }
  ...
  <div class="page">
    ...
    <?php
      $baseQs = function($p) use ($search, $searchActive, $searchCols, $searchCond, $sortQs) {
          ...
          return 'role.php?' . http_build_query($qs);
      };
      ...
      $exportDropdownHtml = '';
      foreach ([
          ['fmt' => 'csv', 'filename' => 'Роли сотрудников.csv', 'format' => 'CSV'],
          ['fmt' => 'xls', 'filename' => 'Роли сотрудников.xls', 'format' => 'XLS (Excel)'],
      ] as $item) {
          $fullUrl = 'role_export.php?format=' . $item['fmt'] . ($exportQs !== '' ? '&' . $exportQs : '');
    ...
    <div class="table-wrap">
      <table class="data-table">
        <?php render_table_colgroup($visibleColumns, $columnWidths, ['id' => '46px', 'role' => '250px', 'note' => '500px']); ?>
        <?php render_table_thead($visibleColumns, $COL_META, $sortLevels, $allRowsMarked, $rowsTotalCount === 0); ?>
        <?php render_table_tbody($visibleColumns, $rows, $marks, $search, 'role_id', function($r, $cn, $vc) use ($searchCond, $searchCols) {
            $search = $GLOBALS['search'] ?? '';
            $doHilight = in_array($cn, $searchCols, true);
            switch ($cn) {
                case 'id':   $raw = (string)(int)$r['role_id']; return [$raw, $raw];
                case 'role': $raw = (string)$r['role']; return [$raw, $doHilight ? hilight($raw, $search, $searchCond) : $raw];
    ...
    (function () {
      ...
      SelectionToolbar.init({
        ...
        getExportUrl: function () {
          ...
          return 'role_export.php?format=csv&selected=1&' + sp.toString();
        },
        getPrintUrl: function () {
          var sp = new URLSearchParams(location.search);
          ['action', 'page'].forEach(function (k) { sp.delete(k); });
          return 'role_print.php?selected=1&' + sp.toString();
        }
      });
  ...
  <script>
    ...
    (function () {
      ...
      if (tbody) {
        tbody.addEventListener('dblclick', function (e) {
          ...
          if (typeof window.__openFormModal === 'function') {
            window.__openFormModal('role_form.php?mode=edit&id=' + id);
          }
        });
      }
      ...
      function openForm(mode) {
        ...
        if (!id) return;
        if (typeof window.__openFormModal === 'function') {
          window.__openFormModal('role_form.php?mode=' + mode + '&id=' + id);
        }
      }
      ...
      function navigate(apply) {
        ...
        apply(p);
        location.href = 'role.php?' + p.toString();
      }

      bindTableKeyboardShortcuts({
        formPrefix: 'role_form',
        rowSel: rowSel,
        currentPage: currentPage,
...
<?= $valCases ?>
    ...
    ColumnResize.init({ saveUrl: 'role_column_width_save.php', tbl: 'role' });
  </script>
