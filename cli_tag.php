...
$tp->appendWhereRaw("tg.client_flag = '1'");
...
$COL_META         = $tp->colMeta;
$columnWidths     = load_columns_widths($conn, 'tag');
...
handle_marks_actions($conn, $tp, 'tag', function($action) use ($conn) {
    return null;
});
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($_GET['action'] ?? '') === 'columnsSave') {
    ...
    $ok = save_columns_config($conn, 'tag', $clean);
...
$rowsMarkedCount = 0;
foreach ($rows as $r) { if (isset($marks[(int)$r['tag_id']])) $rowsMarkedCount++; }
...
?><?php
render_head_start('Виды деятельности');
?>
  <style>
    .data-table tbody tr:hover td.col-tag { background: #2c3a4d; }
    .data-table tbody tr.selected td.col-tag { background: #3a5a8a; }
    .data-table tbody tr.selected:hover td.col-tag { background: #3a5a8a; }
  </style>
<?php
render_export_modal();
render_head_end(); ?>
  <div class="page">
    <?php $activeMenu = 'cli_tag.php'; include 'menu.php'; ?>

    <h1 class="page-title"><img src="img/cli_tag.png" alt="" /> Вид деятельности</h1>
    ...
    <?php
      $baseQs = function($p) use ($search, $searchActive, $searchCols, $searchCond, $sortQs) {
          ...
          return 'cli_tag.php?' . http_build_query($qs);
      };
    ...
    <?php
      $exportDropdownHtml = '';
      foreach ([
          ['fmt' => 'csv', 'filename' => 'Виды деятельности.csv', 'format' => 'CSV'],
          ['fmt' => 'xls', 'filename' => 'Виды деятельности.xls', 'format' => 'XLS (Excel)'],
          ['fmt' => 'pdf', 'filename' => 'Виды деятельности.csv', 'format' => 'PDF'],
      ] as $item) {
          $fullUrl = 'cli_tag_export.php?format=' . $item['fmt'] . ($exportQs !== '' ? '&' . $exportQs : '');
      ...
      $printDropdownHtml = render_print_dropdown_items('cli_tag', $exportQs, (int)$page, $marksCount > 0);
    ?>
    ...
    render_toolbar_left('cli_tag_form', $marksCount, $exportDropdownHtml, $printDropdownHtml);
    render_toolbar_right($search, $searchActive, $urlCols, $searchCond, $clearQs, 'cli_tag.php');
    ...
    <div class="table-wrap">
      <table class="data-table">
        <?php render_table_colgroup($visibleColumns, $columnWidths, ['id' => '46px', 'tag' => '300px', 'note' => '500px']); ?>
        <?php render_table_thead($visibleColumns, $COL_META, $sortLevels, $allRowsMarked, $rowsTotalCount === 0); ?>
        <?php render_table_tbody($visibleColumns, $rows, $marks, $search, 'tag_id', function($r, $cn, $vc) use ($searchCond, $searchCols) {
            $search = $GLOBALS['search'] ?? '';
            $doHilight = in_array($cn, $searchCols, true);
            switch ($cn) {
                case 'id':
                    $raw = (string)(int)$r['tag_id'];
                    return [$raw, $raw];
                case 'tag':
                    $raw = (string)$r['tag'];
...
?>
  ...
  (function () {
    ...
    SearchPanel.init({
      ...
      onApply: function (state) {
        ...
        params.delete('page');
        location.href = 'cli_tag.php?' + params.toString();
      },
    ...
    ColumnsPanel.init({
      btn: columnsBtn,
      saveUrl: 'cli_tag_columns_save.php',
      tbl: 'tag',
      closeAllPanels: closeAllPanels,
    ...
    window.__columnDefaultWidths = <?= json_encode(['id' => 46, 'tag' => 300, 'note' => 500], JSON_UNESCAPED_UNICODE) ?>;
    ...
    document.querySelectorAll('tbody tr').forEach(function (tr) {
      ...
      tr.addEventListener('dblclick', function () {
        window.__openFormModal('cli_tag_form.php?mode=edit&id=' + id);
      });
    });

    rowOpenBtn.addEventListener('click', function () {
      const id = rowSel.getSelectedId();
      if (id !== 0) window.__openFormModal('cli_tag_form.php?mode=edit&id=' + id);
    });
    rowCopyBtn.addEventListener('click', function () {
      const id = rowSel.getSelectedId();
      if (id !== 0) window.__openFormModal('cli_tag_form.php?mode=copy&id=' + id);
    });
    rowDeleteBtn.addEventListener('click', function () {
      const id = rowSel.getSelectedId();
      if (id !== 0) window.__openFormModal('cli_tag_form.php?mode=delete&id=' + id);
    });
  ...
  InlineEdit.init({
    ...
    saveUrl: 'cli_tag_field_save.php',
    fields: <?php
      $inlineFields = [];
      $valCases = '';
...
<?= $valCases ?>
  ...
  ColumnResize.init({ saveUrl: 'cli_tag_column_width_save.php', tbl: 'tag' });
  </script>
