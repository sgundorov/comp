...
if ($rawCols !== '') {
    $searchCols = array_values(array_filter(array_map('trim', explode(',', $rawCols)), function ($k) {
        return in_array($k, ['city', 'country', 'note'], true);
    }));
}
...
if ($searchActive && $search !== '' && count($searchCols) === 0) {
    $searchCols = ['city', 'country', 'note'];
}

$countryFilter = (string)($_GET['country_id'] ?? '');
$countryIds = [];
if ($countryFilter !== '') {
    $countryIds = array_values(array_filter(array_map('intval', explode(',', $countryFilter)), fn($v) => $v > 0));
}
...
if ($sortRaw !== '') {
    foreach (explode(',', $sortRaw) as $lv) {
        ...
        if (in_array($ck, ['id','city','country','note'], true) && in_array($dk, ['asc','desc'], true)) {
...
$COLUMN_DEFAULTS = city_columns_defaults();
$COL_META = [];
...
$COL_META['id']['value']      = function ($r) { return (int)$r['city_id']; };
$COL_META['city']['value']    = function ($r) { return (string)$r['city']; };
$COL_META['country']['value'] = function ($r) { return (string)($r['country_name'] ?? ''); };
$COL_META['note']['value']    = function ($r) { return (string)$r['note']; };
$PRINT_WIDTHS = city_columns_widths_print();

$columnsConfig  = load_columns_config($conn, 'city', $COLUMN_DEFAULTS);
...
if ($search !== '' && count($searchCols) > 0) {
    ...
    $condLabels = ['contains' => 'Содержит', 'not_contains' => 'Не содержит', 'starts_with' => 'Начинается с', 'ends_with' => 'Заканчивается на', 'equals' => 'Равно', 'not_equals' => 'Не равно'];
    $filterLabels[] = 'Поиск: «' . $search . '» ' . ($condLabels[$searchCond] ?? 'Содержит');
}
if (count($countryIds) > 0) {
    $place = implode(',', array_fill(0, count($countryIds), '?'));
    $extra = "c.country_id IN ($place)";
    $where = $where === '' ? "WHERE $extra" : '(' . substr($where, 6) . ") AND $extra";
    $params = array_merge($params, $countryIds);
    $types  = $types . str_repeat('i', count($countryIds));
    $cnr = @$conn->prepare("SELECT country_id, country FROM country WHERE country_id IN ($place)");
    if ($cnr) {
        $cnr->bind_param(str_repeat('i', count($countryIds)), ...$countryIds);
        $cnr->execute();
        $cnRes = $cnr->get_result();
        $cNames = [];
        if ($cnRes) while ($cr = $cnRes->fetch_assoc()) $cNames[] = $cr['country'];
        $cnr->close();
        $filterLabels[] = 'Страна = ' . implode(', ', $cNames);
    }
}
...
$skipQuery = false;
if ($onlySelected) {
    $marks = load_marks_set($conn, 'city');
...
$pageCount = 0;
if ($onlyPage) {
    $countSql = "SELECT COUNT(*) AS c FROM city c LEFT JOIN country co ON co.country_id = c.country_id $where";
...
$sql = "SELECT c.city_id, c.city, co.country AS country_name, c.note
        FROM city c
...
<body>
  ...
  <div class="print-page">
    <div class="print-page-header">
      <div class="print-page-title">Города<?= $onlyPage ? ' &mdash; страница ' . (int)$pageNum : '' ?></div>
