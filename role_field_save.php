...
$chk = $conn->prepare("SELECT role_id FROM `role` WHERE role_id = ?");
$chk->bind_param('i', $id);
$chk->execute();
$chk->store_result();
...
if ($field === 'role' && $value === '') {
    echo json_encode(['ok' => false, 'error' => 'Поле «Роль» не может быть пустым']);
    exit;
}
if (mb_strlen($value) > $cfg['max']) $value = mb_substr($value, 0, $cfg['max']);

$stmt = $conn->prepare("UPDATE `role` SET {$cfg['col']} = ? WHERE role_id = ?");
