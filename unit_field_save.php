...
$chk = $conn->prepare("SELECT unit_id FROM unit WHERE unit_id = ?");
$chk->bind_param('i', $id);
$chk->execute();
$chk->store_result();
...
if ($field === 'unit' && $value === '') {
    echo json_encode(['ok' => false, 'error' => 'Поле «Ед. измерения» не может быть пустым']);
    exit;
}
if (mb_strlen($value) > $cfg['max']) $value = mb_substr($value, 0, $cfg['max']);

$stmt = $conn->prepare("UPDATE unit SET {$cfg['col']} = ? WHERE unit_id = ?");
