...
$chk = $conn->prepare("SELECT country_id FROM country WHERE country_id = ?");
$chk->bind_param('i', $id);
$chk->execute();
$chk->store_result();
...
$ALLOWED = [
    'country' => ['col' => 'country', 'type' => 's', 'max' => 200],
...
if ($field === 'country' && $value === '') {
    echo json_encode(['ok' => false, 'error' => 'Поле «Страна» не может быть пустым']);
    exit;
}
if (mb_strlen($value) > $cfg['max']) $value = mb_substr($value, 0, $cfg['max']);

$stmt = $conn->prepare("UPDATE country SET {$cfg['col']} = ? WHERE country_id = ?");
