...
if ($rawCols !== '') {
    $searchCols = array_values(array_filter(array_map('trim', explode(',', $rawCols)), function ($k) {
        return in_array($k, ['id', 'contype', 'note'], true);
    }));
}
...
if ($sortRaw !== '') {
    foreach (explode(',', $sortRaw) as $lv) {
        ...
        if (in_array($ck, ['id','contype','impotant_flag','note'], true) && in_array($dk, ['asc','desc'], true)) {
...
$COLUMN_DEFAULTS = contype_columns_defaults();
$COL_META = [];
...
$COL_META['id']['value']             = function ($r) { return (int)$r['contype_id']; };
$COL_META['contype']['value']        = function ($r) { return (string)$r['contype']; };
$COL_META['impotant_flag']['value']  = function ($r) { return (string)($r['impotant_flag'] ?? '0'); };
$COL_META['note']['value']           = function ($r) { return (string)$r['note']; };
$PRINT_WIDTHS = contype_columns_widths_print();

$columnsConfig  = load_columns_config($conn, 'contype', $COLUMN_DEFAULTS);
...
$pageCount = 0;
if ($onlyPage) {
    $countSql = "SELECT COUNT(*) AS c FROM contype ct $where";
...
$sql = "SELECT ct.contype_id, ct.contype, ct.impotant_flag, ct.note
        FROM contype ct
        $where
        ORDER BY $orderBy";
if ($onlyPage) {
...
<head>
  <meta charset="UTF-8" />
  <title>Виды контактов — Печать</title>
  <style>
    body {
...
<body>
  ...
  <div class="print-page">
    <div class="print-page-header">
      <div class="print-page-title">Виды контактов<?= $onlyPage ? ' &mdash; страница ' . (int)$pageNum : '' ?></div>
