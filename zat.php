...
$accessFlags = render_access_control($conn, 'Zat');
...
$rowsMarkedCount = 0;
foreach ($rows as $r) { if (isset($marks[(int)$r['zat_id']])) $rowsMarkedCount++; }
$rowsTotalCount  = count($rows);
$allRowsMarked   = $rowsTotalCount > 0 && $rowsMarkedCount === $rowsTotalCount;
?>
<?php
render_head_start('Операции с деньгами'); ?>
  <style>
    ...
    .col-out_flag { text-align: center; }
    .col-out_flag .check-icon { display: inline-block; vertical-align: middle; }
  </style>
<?php
render_head_end();
render_export_modal();
  ...
  <div class="page">
    ...
    <?php
      $baseQs = function($p) use ($search, $searchActive, $searchCols, $searchCond, $sortQs) {
          ...
          return 'zat.php?' . http_build_query($qs);
      };
      ...
      $exportDropdownHtml = '';
      foreach ([
          ['fmt' => 'csv', 'filename' => 'Операции с деньгами.csv', 'format' => 'CSV'],
          ['fmt' => 'xls', 'filename' => 'Операции с деньгами.xls', 'format' => 'XLS (Excel)'],
      ] as $item) {
          $fullUrl = 'zat_export.php?format=' . $item['fmt'] . ($exportQs !== '' ? '&' . $exportQs : '');
      ...
      $printDropdownHtml = render_print_dropdown_items('zat', $exportQs, (int)$page, $marksCount > 0);
    ?>
    <?php
    ...
    render_toolbar_left('zat_form', $marksCount, $exportDropdownHtml, $printDropdownHtml);
    render_toolbar_right($search, $searchActive, $urlCols, $searchCond, $clearQs, 'zat.php');
    ...
    <div class="table-wrap">
      <table class="data-table">
        <?php render_table_colgroup($visibleColumns, $columnWidths, ['id' => '46px', 'name' => '200px', 'out_flag' => '80px', 'note' => '500px']); ?>
        <?php render_table_thead($visibleColumns, $COL_META, $sortLevels, $allRowsMarked, $rowsTotalCount === 0); ?>
        <?php render_table_tbody($visibleColumns, $rows, $marks, $search, 'zat_id', function($r, $cn, $vc) use ($searchCond, $searchCols) {
            $search = $GLOBALS['search'] ?? '';
            $doHilight = in_array($cn, $searchCols, true);
            switch ($cn) {
                case 'id':      $raw = (string)(int)$r['zat_id']; return [$raw, $raw];
                case 'name':    $raw = (string)$r['name']; return [$raw, $doHilight ? hilight($raw, $search, $searchCond) : $raw];
                case 'out_flag': $v = (int)($r['out_flag'] ?? 0); return [(string)$v, $v ? '<svg class="check-icon" viewBox="0 0 24 24" width="16" height="16"><path fill="#27ae60" d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>' : ''];
...
?>
  <script>
    ...
    window.__columnDefaultWidths = <?= json_encode(['id' => 46, 'name' => 200, 'out_flag' => 80, 'note' => 500], JSON_UNESCAPED_UNICODE) ?>;
  ...
  <script>
    ...
    (function () {
      SelectionToolbar.initTableSelection('zat.php', document.querySelector('.toolbar').getAttribute('data-search') || '');
      ...
      SelectionToolbar.init({
        ...
        getExportUrl: function () {
          ...
          return 'zat_export.php?format=csv&selected=1&' + sp.toString();
        },
        getPrintUrl: function () {
          var sp = new URLSearchParams(location.search);
          ['action', 'page'].forEach(function (k) { sp.delete(k); });
          return 'zat_print.php?selected=1&' + sp.toString();
        }
      });
  ...
  <script>
    ...
    (function () {
      ...
      if (tbody) {
        tbody.addEventListener('dblclick', function (e) {
          ...
          if (typeof window.__openFormModal === 'function') {
            window.__openFormModal('zat_form.php?mode=edit&id=' + id);
          }
        });
      }
      ...
      function openForm(mode) {
        ...
        if (!id) return;
        if (typeof window.__openFormModal === 'function') {
          window.__openFormModal('zat_form.php?mode=' + mode + '&id=' + id);
        }
      }
      ...
      function navigate(apply) {
        ...
        apply(p);
        location.href = 'zat.php?' + p.toString();
      }
...
<?= $valCases ?>
    ...
    ColumnResize.init({ saveUrl: 'zat_column_width_save.php', tbl: 'zat' });
  </script>
