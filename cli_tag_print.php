...
if ($rawCols !== '') {
    $searchCols = array_values(array_filter(array_map('trim', explode(',', $rawCols)), function ($k) {
        return in_array($k, ['id', 'tag', 'note'], true);
    }));
}
...
if ($sortRaw !== '') {
    foreach (explode(',', $sortRaw) as $lv) {
        ...
        if (in_array($ck, ['id','tag','note'], true) && in_array($dk, ['asc','desc'], true)) {
...
$COLUMN_DEFAULTS = tag_columns_defaults();
$COL_META = [];
...
$COL_META['id']['value']   = function ($r) { return (int)$r['tag_id']; };
$COL_META['tag']['value']  = function ($r) { return (string)$r['tag']; };
$COL_META['note']['value'] = function ($r) { return (string)$r['note']; };
$PRINT_WIDTHS = tag_columns_widths_print();

$columnsConfig  = load_columns_config($conn, 'tag', $COLUMN_DEFAULTS);
$visibleColumns = array_values(array_filter($columnsConfig, function ($c) { return !empty($c['visible']); }));

$where  = "tg.client_flag = '1'";
$params = [];
$types  = '';
if ($search !== '' && count($searchCols) > 0) {
    ...
    if (count($parts) > 0) {
        $where = '(' . $where . ') AND (' . implode(' OR ', $parts) . ')';
    }
}
...
$pageCount = 0;
if ($onlyPage) {
    $countSql = "SELECT COUNT(*) AS c FROM tag tg WHERE $where";
...
$sql = "SELECT tg.tag_id, tg.tag, tg.note
        FROM tag tg
        WHERE $where
        ORDER BY $orderBy";
if ($onlyPage) {
...
<head>
  <meta charset="UTF-8" />
  <title>Виды деятельности — Печать</title>
  <style>
    body {
...
<body>
  ...
  <div class="print-page">
    <div class="print-page-header">
      <div class="print-page-title">Виды деятельности<?= $onlyPage ? ' &mdash; страница ' . (int)$pageNum : '' ?></div>
