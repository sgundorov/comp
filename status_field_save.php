...
$chk = $conn->prepare("SELECT status_id FROM status WHERE status_id = ?");
$chk->bind_param('i', $id);
$chk->execute();
$chk->store_result();
...
$ALLOWED = [
    'status' => ['col' => 'status', 'type' => 's', 'max' => 200],
...
if ($field === 'status' && $value === '') {
    echo json_encode(['ok' => false, 'error' => 'Поле «Статус» не может быть пустым']);
    exit;
}
if (mb_strlen($value) > $cfg['max']) $value = mb_substr($value, 0, $cfg['max']);

$stmt = $conn->prepare("UPDATE status SET {$cfg['col']} = ? WHERE status_id = ?");
