...
if (($mode === 'edit' || $mode === 'copy' || $mode === 'delete') && $id > 0) {
    $stmt = $conn->prepare("SELECT status, color, note FROM status WHERE status_id = ?");
    ...
    if (!$r) {
        ...
        $origStatus = (string)$r['status'];
        $values['status'] = ($mode === 'copy') ? '' : (string)$r['status'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($mode !== 'delete' && $values['status'] === '') {
        $errors[] = 'Поле «Статус» обязательно для заполнения.';
        $focusField = 'status';
    }
    ...
    if (empty($errors)) {
        if ($mode === 'delete') {
            $stmt = $conn->prepare("DELETE FROM status WHERE status_id = ?");
            $stmt->bind_param('i', $id);
            $stmt->execute();
            $stmt->close();
            $conn->query("DELETE FROM marks WHERE tbl = 'status' AND row_id = " . (int)$id);
            if ($isAjax) {
                header('Content-Type: application/json; charset=utf-8');
                echo json_encode(['ok' => true, 'mode' => $mode, 'id' => $id, 'name' => $origStatus]);
                exit;
            }
        ...
        if ($mode === 'new' || $mode === 'copy') {
            $stmt = $conn->prepare("INSERT INTO status (status, color, note) VALUES (?, ?, ?)");
            $colorInt = $values['color'] ? (int)hexdec(ltrim($values['color'], '#')) : 0;
            bind_auto($stmt, [$values['status'], $colorInt, $values['note']]);
            if (!$stmt->execute()) {
            ...
            $stmt->close();
            if ($isAjax) {
                $pageOfNew = computePageOfNew($conn, 'status', 'id', 'id', 'asc', $newId, $newId, '');
                header('Content-Type: application/json; charset=utf-8');
                echo json_encode(['ok' => true, 'mode' => $mode, 'id' => $newId, 'name' => $values['status'], 'page' => $pageOfNew]);
                exit;
            }
            header('Location: status.php');
            exit;
        ...
        if ($mode === 'edit') {
            $stmt = $conn->prepare("UPDATE status SET status = ?, color = ?, note = ? WHERE status_id = ?");
            $colorInt = $values['color'] ? (int)hexdec(ltrim($values['color'], '#')) : 0;
            bind_auto($stmt, [$values['status'], $colorInt, $values['note'], $id]);
            if (!$stmt->execute()) {
                $errors[] = 'Ошибка БД: ' . $stmt->error;
                $stmt->close();
                goto render;
            ...
            if ($isAjax) {
                header('Content-Type: application/json; charset=utf-8');
                echo json_encode(['ok' => true, 'mode' => $mode, 'id' => $id, 'name' => $values['status']]);
                exit;
            }
            header('Location: status.php');
            exit;
...
render:
$titles = [
    'new'    => 'Состояние заявки (новое)',
    'edit'   => 'Состояние заявки: ' . $origStatus,
    'copy'   => 'Состояние заявки: ' . $origStatus . ' (копия)',
    'delete' => 'Состояние заявки: ' . $origStatus . ' (удаление)',
];
$pageTitle = $titles[$mode] ?? 'Состояние заявки';
$isReadonly = ($mode === 'delete');

ob_start();
?>
<h2 class="page-title<?= $mode === 'delete' ? ' page-title--delete' : '' ?>"><img src="img/status.png" alt="" /> <?= h($pageTitle) ?></h2>
<form class="form<?= $mode === 'delete' ? ' form--delete' : '' ?>" method="post" action="status_form.php" autocomplete="off" data-form-modal>
...
<?= render_form_actions(
    $mode === 'delete'
        ...
        : [render_btn_primary('img/save.png', 'Сохранить', ['type'=>'submit','formnovalidate'=>true,'tabindex'=>'-1']),
           render_btn_link_icon_text('img/cancel.png', 'Отменить', 'status.php', ['class'=>'btn-secondary','tabindex'=>'-1'])]
) ?>
<?php endif; ?>

<?= render_field('Статус', render_input('text', 'status', $values['status'], [
        'id' => 'status-name',
        'required' => !$isReadonly,
        'readonly' => $isReadonly,
...
<?= render_field('Цвет выделения', render_color_picker('color', $values['color'], $isReadonly), false, ['readonly' => $isReadonly]) ?>
...
<?= render_form_actions(
    $mode === 'delete'
        ? [render_btn_danger('img/delete.png', 'Удалить', ['type'=>'submit','formnovalidate'=>true]),
           render_btn_link_icon_text('img/cancel.png', 'Отменить', 'status.php', ['class'=>'btn-secondary'])]
